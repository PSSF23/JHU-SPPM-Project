...
switch flag
    case 1
        ...
        for j = 1:5 %loop through 3 dosing methods
            y0 = [q(j) / p.v1, 0, 0];
            [T, Ytemp, AUC4Wb(j), AUC12Wb(j), Cmaxb(j)] = Sen_Simulation_2C(p, y0, TimeLen, Interval(j), q(j));
            y = [y, Ytemp(:, 1)];
        ...
        for i = 1:numel(params)
            for m = 1:5 % three dosing methods
                ...
                [ttemp, ytemp, AUC4W(i, m), AUC12W(i, m), Cmax(i, m)] = Sen_Simulation_2C(pnew, y0, TimeLen, Interval(m), qnew(m));
                y = [y, ytemp(:, 1)];
        ...
        save data/Sens_AUC4W.mat Sens_AUC4W
        %save the sensitivity for AUC in first 12 weeks
        save data/Sens_AUC12W.mat Sens_AUC12W
        % save the sensitivity for Cmax
        save data/Sens_Cmax.mat Sens_Cmax
        % save all the data points
        save data/Sens_expdata.mat t y
    ...
    case 2
        ...
        for j = 1:5 % loop through 5 different methods
            ...
            for i = 1:length(Albumin) %21 values
                ...
                y0 = [q(j) / pnew.v1, 0, 0];
                [~, ~, AUC4Wb(i), AUC12Wb(i), Cmaxb(i)] = Sen_Simulation_2C(pnew, y0, TimeLen, Interval(j), q(j));
                ...
                for k = 1:numel(params) % 8 parameters
                    ...
                    [~, ~, AUC4W(i, k), AUC12W(i, k), Cmax(i, k)] = Sen_Simulation_2C(pnew, y0, TimeLen, Interval(j), qnew(j));
        ...
        %save the sensitivity for AUC in first 4 weeks
        save data/Sensg_AUC4W_A.mat Sensg_AUC4W_A
        %save the sensitivity for AUC in first 12 weeks
        save data/Sensg_AUC12W_A.mat Sensg_AUC12W_A
        % save the sensitivity for Cmax
        save data/Sensg_Cmax_A.mat Sensg_Cmax_A
        
        %save the sensitivity for AUC in first 4 weeks
        save data/Sensg_AUC4W_W.mat Sensg_AUC4W_W
        %save the sensitivity for AUC in first 12 weeks
        save data/Sensg_AUC12W_W.mat Sensg_AUC12W_W
        % save the sensitivity for Cmax
        save data/Sensg_Cmax_W.mat Sensg_Cmax_W
        ...
        for j = 1:5 % loop through 5 different methods
            ...
            for i = 1:length(Albumin) %loop with 21 values of albumin
                ...
                for k = 1:length(Weight)
                    ...
                    qnew = [(2 * pnew.weight), (10 * pnew.weight), (10 * pnew.weight), 200, 400]; % [2mg weight based; 10mg weight based; 200mg fixed dose]
                    y0 = [qnew(j) / pnew.v1, 0, 0];
                    [~, ~, AUC4Wgt(k, i), AUC12Wgt(k, i), Cmaxgt(k, i)] = Sen_Simulation_2C(pnew, y0, TimeLen, Interval(j), qnew(j));
                end
