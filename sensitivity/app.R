...
column_names <- c(
  "base, 2mg Q3W", "base, 10mg Q2W", "base, 10mg Q3W", "base, 200mg Q3W",
...
Parameter <- c(
  "weight", "eGFR", "ALB", "BSLD", "Qt", "kdegD", "kon", "koff", "Emaxtp",
...
Local3C$Parameter <- factor(Local3C$Parameter, levels = c(
  "weight", "eGFR", "ALB", "BSLD", "Qt", "kdegD", "kon", "koff", "Emaxtp",
  "EC50tp", "NTc", "PD1Tc", "PercT_PD1", "Tmulti", "gamma"
))
...
column_names_A <- c(
  "Albumin", "cl, 2mg Q3W", "cl, 10mg Q2W", "cl, 10mg Q3W", "cl, 200mg Q3W",
  "cl, 400mg Q6W", "v1, 2mg Q3W", "v1, 10mg Q2W", "v1, 10mg Q3W", "v1, 200mg Q3W",
  "v1, 400mg Q6W", "v2, 2mg Q3W", "v2, 10mg Q2W", "v2, 10mg Q3W", "v2, 200mg Q3W",
  "v2, 400mg Q6W", "Q, 2mg Q3W", "Q, 10mg Q2W", "Q, 10mg Q3W", "Q, 200mg Q3W", "Q, 400mg Q6W",
  "weight, 2mg Q3W", "weight, 10mg Q2W", "weight, 10mg Q3W", "weight, 200mg Q3W",
...
column_names_W <- c(
  "Weight", "cl, 2mg Q3W", "cl, 10mg Q2W", "cl, 10mg Q3W", "cl, 200mg Q3W",
  "cl, 400mg Q6W", "v1, 2mg Q3W", "v1, 10mg Q2W", "v1, 10mg Q3W", "v1, 200mg Q3W",
  "v1, 400mg Q6W", "v2, 2mg Q3W", "v2, 10mg Q2W", "v2, 10mg Q3W", "v2, 200mg Q3W",
  "v2, 400mg Q6W", "Q, 2mg Q3W", "Q, 10mg Q2W", "Q, 10mg Q3W", "Q, 200mg Q3W", "Q, 400mg Q6W",
  "weight, 2mg Q3W", "weight, 10mg Q2W", "weight, 10mg Q3W", "weight, 200mg Q3W",
...
globe_A <- separate(globe_A,
  col = "Dosing",
  into = c("Parameter", "Dosing"),
  sep = ", "
)
globe_W <- separate(globe_W,
  col = "Dosing",
  into = c("Parameter", "Dosing"),
  sep = ", "
)
...
## ==============================================================================
# load data for 2-dimensional global sensitivity
...
column_names_g <- c(
  "Weight", "12, 2mg Q3W", "14, 2mg Q3W", "17, 2mg Q3W", "19, 2mg Q3W",
...
AUC12W3C_g <- readMat("data/Cmax3C_g.mat")
...
repeated <- c("Weight", "Dosing")
Globe3C <- Local3C <- merge(merge(merge(AUC12W3C_g, Cmax3C_g, by = repeated), Vtumor3C_g, by = repeated), Dtumor3C_g, by = repeated)

names(Globe3C) <- c("Weight", "Dosing", "AUC12W", "Cmax", "Vtumor", "Dtumor")
Globe3C <- separate(Globe3C,
  col = "Dosing",
  into = c("Albumin", "Dosing"),
  sep = ", "
)
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Local Sensitivity 2C",
        tabName = "local", icon = icon("dashboard"),
      ...
      menuItem("Global Sensitivity 2C",
        tabName = "global", icon = icon("dashboard"),
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          ...
          box(
            title = "Heat maps", width = 9, solidHeader = TRUE, status = "primary",
      ...
      tabItem(
        ...
        box(
          title = "Sensitivitiy Comparison", width = 9, solidHeader = TRUE, status = "primary",
        ...
        fluidRow(
          box(
            ...
            selectInput(
              ...
              choices = c(
                "The area under curve for first 4 weeks" = "AUC4",
        ...
        fluidRow(
          box(
            ...
            selectInput(
              ...
              label = "Choose which changing outcome to display:",
              choices = c(
                "Central Compartment AUC for 12 weeks" = "AUC12W",
...
server <- function(input, output) {
  ...
  output$localSensPlot1 <- renderPlotly({
    ...
    localSensPlot1 <- ggplot(data = AUC4W_filtered, aes(x = Dosing, y = Parameter, fill = Sens)) +
      geom_tile() +
      scale_fill_distiller(name = "Sensitivity", palette = "YlGnBu", limits = c(-1, 1.5)) +
      labs(
  ...
  output$localSensPlot2 <- renderPlotly({
    ...
    localSensPlot2 <- ggplot(data = AUC12W_filtered, aes(x = Dosing, y = Parameter, fill = Sens)) +
      geom_tile() +
      scale_fill_distiller(name = "Sensitivity", palette = "YlGnBu", limits = c(-1, 1.1)) +
      labs(
  ...
  output$localSensPlot3 <- renderPlotly({
    ...
    localSensPlot3 <- ggplot(data = Cmax_filtered, aes(x = Dosing, y = Parameter, fill = Sens)) +
      geom_tile() +
      scale_fill_distiller(name = "Sensitivity", palette = "YlGnBu", limits = c(-1, 2)) +
      labs(
  ...
  output$Plotconc <- renderPlotly({
    ...
    Plotconc <- Plotconc + geom_line(
      ...
      scale_color_brewer(name = "", palette = "Set2") +
      labs(
        title = "Pembrolizumab Concentration-time Profile",
  ...
  output$glo_para_A <- renderPlotly({
    ...
    glo_para_A <- ggplot(data = globe_A_filtered, aes(x = Albumin, y = Parameter, fill = Sens)) +
      geom_tile() +
      scale_fill_distiller(name = "Sensitivity", palette = "YlGnBu", limits = c(min(globe_A_filtered["Sens"]), max(globe_A_filtered["Sens"]))) +
      labs(
        title = "Parameters VS Global Sensitivity of Albumin",
  ...
  output$glo_para_W <- renderPlotly({
    ...
    glo_para_W <- ggplot(data = globe_W_filtered, aes(x = Weight, y = Parameter, fill = Sens)) +
      geom_tile() +
      scale_fill_distiller(name = "Sensitivity", palette = "YlGnBu", limits = c(min(globe_W_filtered["Sens"]), max(globe_W_filtered["Sens"]))) +
      labs(
        title = "Parameters VS Global Sensitivity of Weight",
        x = "Body weights, kg",
        y = "Parameters"
      ) +
      heatmap_theme
  ...
  output$globe <- renderPlotly({
    ...
    globe <- ggplot(data = Globe_filtered, aes(x = Albumin, y = Weight, fill = Sens)) +
      ...
      labs(
        title = "Two-dimensional Global Senstivity",
        x = "Albumin (g/L)",
        y = "Weight(kg)"
      ) +
      heatmap_theme
  ...
  output$localSensPlot <- renderPlotly({
    ...
    localSensPlot <- ggplot(data = Local3C_filtered, aes(x = Dosing, y = Parameter, fill = Sens)) +
      ...
      scale_fill_distiller(
        name = "Sensitivity", palette = "YlGnBu",
        limits = c(min(Local3C_filtered["Sens"]), max(Local3C_filtered["Sens"]))
      ) +
      labs(
        title = "Parameter Local Sensitivity",
        x = "Dosing Methods"
      ) +
      heatmap_theme
    # Convert ggplot to Plotly plot
