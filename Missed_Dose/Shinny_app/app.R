...
retake_2C<-process_data_retake_2C(list(retake_2C_2mg_Q3W,retake_2C_10mg_Q3W,retake_2C_10mg_Q2W,
                                  retake_2C_200mg_Q3W,retake_2C_400mg_Q6W),dosing_tag,retake_tag)
...
full_3C<-process_data_3C(list(Full_3C_2mg_Q3W,Full_3C_10mg_Q3W,Full_3C_10mg_Q2W,Full_3C_200mg_Q3W,Full_3C_400mg_Q6W),
                         dosing_tag,scheme = 'full')
#Read missed dose data
...
miss_3C<-process_data_3C(list(miss_3C_2mg_Q3W,miss_3C_10mg_Q3W,miss_3C_10mg_Q2W,miss_3C_200mg_Q3W,miss_3C_400mg_Q6W),
                         dosing_tag = dosing_tag,scheme = 'miss')
#Read retaken dose data
...
retake_3C<-process_data_retake_3C(list(retake_3C_2mg_Q3W,retake_3C_10mg_Q3W,retake_3C_10mg_Q2W,
                                       retake_3C_200mg_Q3W,retake_3C_400mg_Q6W),dosing_tag,retake_tag)

# Combine AUC, Cmax, Ctrough, DrugComplex_min, Tumor_Ratio
AUC_3C<-rbind(full_3C[['AUC']],miss_3C[['AUC']],retake_3C[['AUC']])
Cmax_3C<-rbind(full_3C[['Cmax']],miss_3C[['Cmax']],retake_3C[['Cmax']])
Ctrough_3C<-rbind(full_3C[['Ctrough']],miss_3C[['Ctrough']],retake_3C[['Ctrough']])
colnames(retake_3C[['DrugComplex_min']])<-c("Scheme","DrugComplex_min","Dosing")
DCmin_3C<-rbind(full_3C[['DrugComplex_min']],miss_3C[['DrugComplex_min']],retake_3C[['DrugComplex_min']])
colnames(retake_3C[['Tumor_Ratio']])<-c("Scheme","Tumor_Ratio","Dosing")
TumorR_3C<-rbind(full_3C[['Tumor_Ratio']],miss_3C[['Tumor_Ratio']],retake_3C[['Tumor_Ratio']])
...
#   my_theme+
#   theme(legend.position = "none")+
#   theme(axis.text.x = element_text(size = 16,angle=0,margin = margin(-5,0,0,0)),
...
#   facet_grid(.~Dosing)+
#   geom_boxplot(aes(fill = Scheme),width = 0.25,alpha = 0.4)+
#   labs(y = "Drug Complex Ratio (%)", x = "Schemes",title = 'Mininum Drug Complex Ratio for 3C models')+
#   my_theme+
#   theme(legend.position = "none")+
#   theme(axis.text.x = element_text(size = 16,angle=0))
# DCmin_3C_plot
...
#   facet_grid(.~Dosing)+
#   geom_boxplot(aes(fill = Scheme),width = 0.25,alpha = 0.4)+
#   labs(y = "Tumor change ratio", x = "Schemes",title = 'Tumor Change Ratio for 3C models')+
#   my_theme+
#   theme(legend.position = "none")+
#   theme(axis.text.x = element_text(size = 16,angle=0))
# TumorR_3C_plot
...
#Full and miss
T_full_2C<-as.data.frame(cbind(T_2C[[1]], 'Scheme' = rep('full',times = nrow(T_2C[[1]]))))
T_miss_2C<-as.data.frame(cbind(T_2C[[2]], 'Scheme' = rep('miss',times = nrow(T_2C[[2]]))))
...
T_retake_2C<-pivot_longer(T_retake_2C,cols = everything(),names_to = 'Scheme',values_to = 'Time')
Y_retake_2C_new<-as.data.frame(c(Y_retake_2C$'1/5',Y_retake_2C$'2/5',Y_retake_2C$'3/5',Y_retake_2C$'4/5',Y_retake_2C$'5/5'))
...
#Full and miss
Y_full_3C<-as.data.frame(cbind((Y_3C[[1]]),"Scheme" = rep("full",times=nrow(Y_3C[[1]])) ))
Y_miss_3C<-as.data.frame(cbind((Y_3C[[2]]),"Scheme" = rep("miss",times=nrow(Y_3C[[2]])) ))
...
YT_3C[, columns] <- lapply(columns, function(x) as.numeric(YT_3C[[x]]))
...
sidebar<-
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Description",tabName = "description",icon = icon('question-circle')),
      menuItem('Missed Dose 2C Results',tabName = '2C_result',icon = icon('dashboard'),
               menuSubItem('Typical Concentration-Time Profile',tabName = '2C_typical'),
               menuSubItem('Population Variation',tabName = '2C_pop')
               ),
      menuItem('Missed Dose 3C Reults',tabName = '3C_result',icon = icon('dashboard'),
...
body<-
  ...
  tabItem(tabName = 'description',
          fluidRow(column(
            ...
            tags$p("In this analysis, pembrolizumab is prescribed to random simulated patients to analyze
            ...
            br(),
            h3("Pembrolizumab Information"),
            tags$p("Pembrolizumab is a monoclonal IgG antibody that binds with PD-1 receptors,",
            " and thus inhibits the PD-1/PD-L1 pathway, by which cancer cells use to downregulate T cell",
               ...
               " lead to inhibited tumor growth. For more information, see this article:",
                    tags$b("A Review About Pembrolizumab in First-Line Treatment of Advanced NSCLC: Focus on KEYNOTE Studies"),
                    tags$a(tags$i('PMID: 32801888(https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7395702/)'))
                    ),
            br(),
            h3("5 Dosing regimens"),
            tags$p("We selected 5 dosing regimens. 3 are weight-based dosing selected from",
              tags$a("KEYNOTE-001, 002,006 and 010"),
                    " studies. 2 are fixed dosing recommended by ",
              tags$a("Keytruda.")),
            tags$li("3 weight-based dosing: 2mg/kg Q3W (Once every 3 weeks), 10mg/kg Q3W, 10mg/kg Q2W"),
            tags$li("2 fixed dosing: 200mg Q3W, 400mg Q6W"),
            br(),
            h3("Missed dose scenarios"),
            tags$li("'full' is a positive control, where patients take all doses strictly as scheduled. "),
            tags$li("'miss' refers to cases where patients miss the 2nd dose."),
            tags$li("Retaken dose are cases where a patient miss the 2nd dose but retake the dose
            ...
            br(),
            h3("Models and outputs"),
            tags$p("Two models are used. The 2 compartmental PK model is adapted from the paper
                   ",
                   tags$b("Model-Based Characterization of the Pharmacokinetics of Pembrolizumab: A Humanized Anti-PD-1 Monoclonal Antibody in Advanced Solid Tumors"),
                   tags$a(tags$i("PMID: 27863186. https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5270291/#psp412139-note-0006. ")),
                   ),
            tags$p("The 3 compartmental PK/PD model reserves some components from the 2 compartment
                   PK model. The PD component is adapted from the paper",
                   tags$b("Translational Pharmacokinetic/Pharmacodynamic Modeling of Tumor Growth Inhibition Supports Dose-Range Selection of the Anti-PD-1 Antibody Pembrolizumab"),
                   tags$a(tags$i("PMID: 27863176. https://pubmed.ncbi.nlm.nih.gov/27863176/. "))
                   ),
            tags$li("Outputs for 2 compartment models: Area under curve of pembrolizumab concentration in central compartment over 12
                    weeks (12W AUC); Cmax and Ctrough of central compartment."),
            tags$li("Outputs for 3 compartment models: 12W AUC, Cmax, Ctrough, minimum drug complex ratio (drug-PD-1 complex/total PD-1),
                    Tumor change ratio: (Volume change ratio Vafter/Vbefore - 1)")
          ))
          ),
  ...
  tabItem(tabName = '2C_typical',
          fluidRow(
            box(
              title = 'Parameter Selection', width = 2, solidHeader = TRUE,status = 'primary',
              checkboxGroupInput(
            ...
            box(
              ...
              width = 10, solidHeader = TRUE,status = 'primary',
              fluidRow(column(10,
                              h4("The following is the time-concentration curve for central compartment over a 12-week time span."))),
  ...
  tabItem(tabName = '2C_pop',
          fluidRow(
            box(
              ...
              checkboxGroupInput(
                ...
                choices = c(
                  ...
                  '200mg Q3W' = '200mg Q3W',
                  '400mg Q6W' = '400mg Q6W'),
                selected = c('2mg/kg Q3W', '10mg/kg Q3W', '10mg/kg Q2W', '200mg Q3W','400mg Q6W')
              ),
              checkboxGroupInput(
                inputId = '2C_scheme',
                label = 'Choose drug taking schemes:',
            ...
            box(
              ...
              width = 10, solidHeader = TRUE, status = 'primary',
              plotlyOutput(height = '800px',outputId = '2C_AUC'),
              hr(style = "border-top: 5px solid #3C8DBC ;"),
              plotlyOutput(height = '800px',outputId = '2C_Cmax'),
              hr(style = "border-top: 5px solid #3C8DBC ;"),
              plotlyOutput(height = '800px',outputId = '2C_Ctrough')
            )
          )
          ),
          ...
          fluidRow(
            ...
            box(
              ...
              fluidRow(column(10,
                              ...
                              tags$li("Second,drug complex ratio (drug-PD1 complex/total PD1) time profile. "),
  ...
  tabItem(tabName = '3C_pop',
          fluidRow(
            ...
            box(
              ...
              width = 10, solidHeader = TRUE, status = 'primary',
              plotlyOutput(height = '800px',outputId = '3C_AUC'),
              hr(style = "border-top: 5px solid #3C8DBC ;"),
              plotlyOutput(height = '800px',outputId = '3C_Cmax'),
              hr(style = "border-top: 5px solid #3C8DBC ;"),
              plotlyOutput(height = '800px',outputId = '3C_Ctrough'),
              hr(style = "border-top: 5px solid #3C8DBC ;"),
              plotlyOutput(height = '800px',outputId = '3C_DCmin'),
              hr(style = "border-top: 5px solid #3C8DBC ;"),
              plotlyOutput(height = '800px',outputId = '3C_TumorR')
            )
          ))
)
)
...
server<-function(input,output){
  #========== 2C Typical Data ==========
  YT_2C_selected<-reactive(filter(YT_2C, Scheme %in% input$'2C_typical_scheme'))
  #========= 2C Typical plots ==========
  output$'2C_typical_plot'<-renderPlotly({
    typi_2C<-ggplot(YT_2C_selected(),aes(x = Time/7,y = D1,color = Scheme))+
      geom_line(size= 1.3,alpha = 0.8)+
      scale_color_brewer(name = '',palette = 'Set1')+
      scale_x_continuous(limits = c(0,12),breaks = seq(0,12,2))+
  ...
  #========== 2C Pop Data ==============
  AUC_2C_selected<-reactive(filter(AUC_2C,
                                   Dosing %in% input$'2C_dosing'& 
                                   Scheme %in% input$'2C_scheme'))
  Cmax_2C_selected<-reactive(filter(Cmax_2C,
                                    Dosing %in% input$'2C_dosing'& 
                                    Scheme %in% input$'2C_scheme'))
  Ctrough_2C_selected<-reactive(filter(Ctrough_2C,
                                       Dosing %in% input$'2C_dosing'& 
                                       Scheme %in% input$'2C_scheme'))
  #========= 2C Pop plots ====================
    output$'2C_AUC'<-renderPlotly({
      plot_AUC_2C<-ggplot(AUC_2C_selected(),aes(x = Scheme,y = AUC))+
        ...
        my_theme+
        theme(legend.position = "none")+
        theme(axis.text.x = element_text(size = 11,angle=45,margin = margin(0,0,0,0)),
      ...
      plot_AUC_2C<-plot_AUC_2C%>%plotly_build()%>%
        layout(yaxis = list(title = list(text = 'AUC (mg*Day/L)',
                                         standoff = 2,
                                         font = list(size = 25))),
               xaxis = list(title = list(text = '',standoff = 10,
                                         font = list(size = 10)))
               )
    })
    ...
    output$'2C_Cmax'<-renderPlotly({
      ...
      plot_Cmax_2C<-plot_Cmax_2C%>%plotly_build()%>%
        layout(yaxis = list(title = list(text = 'Cmax (mg/L)',
                                         standoff = 2,
                                         font = list(size = 25))),
               xaxis = list(title = list(text = '',standoff = 10,
                                         font = list(size = 10)))
    ...
    output$'2C_Ctrough'<-renderPlotly({
      plot_Ctrough_2C<-ggplot(Ctrough_2C_selected(),aes(x = Scheme,y = Ctrough))+
        geom_violin(aes(fill = Scheme),scale = "width",alpha = 0.4)+
        facet_grid(.~Dosing)+
        geom_boxplot(aes(fill = Scheme),width = 0.25,alpha = 0.4)+
      ...
      plot_Ctrough_2C<-plot_Ctrough_2C%>%plotly_build()%>%
        layout(yaxis = list(title = list(text = 'Ctrough (mg/L)',
                                         standoff = 2,
                                         font = list(size = 25))),
               xaxis = list(title = list(text = '',standoff = 10,
                                         font = list(size = 10)))
    ...
    #========== 3C Typical Data ==========
    YT_3C_selected<-reactive(filter(YT_3C, Scheme %in% input$'3C_typical_scheme'))
    #========= 3C Typical plots ==========
    output$'3C_D1_plot'<-renderPlotly({
      D1_3C<-ggplot(YT_3C_selected(),aes(x = Time/7,y = D1,color = Scheme))+
        geom_line(size= 1.3,alpha = 0.8)+
        scale_color_brewer(name = '',palette = 'Set1')+
        scale_x_continuous(limits = c(0,12),breaks = seq(0,12,2))+
    ...
    output$'3C_DC_plot'<-renderPlotly({
      DC_3C<-ggplot(YT_3C_selected(),aes(x = Time/7,y = DrugComplex,color = Scheme))+
        geom_line(size= 1.3,alpha = 0.8)+
        scale_color_brewer(name = '',palette = 'Set1')+
        scale_x_continuous(limits = c(0,12),breaks = seq(0,12,2))+
    ...
    output$'3C_TumorV_plot'<-renderPlotly({
      TumorV_3C<-ggplot(YT_3C_selected(),aes(x = Time/7,y = TumorV/1000,color = Scheme))+
        geom_line(size= 1.3,alpha = 0.8)+
        scale_color_brewer(name = '',palette = 'Set1')+
        scale_x_continuous(limits = c(0,12),breaks = seq(0,12,2))+
    ...
    #================= 3C Pop Data =========================
    AUC_3C_selected<-reactive(filter(AUC_3C,
                                     Dosing %in% input$'3C_dosing'& 
                                       Scheme %in% input$'3C_scheme'))
    Cmax_3C_selected<-reactive(filter(Cmax_3C,
                                      Dosing %in% input$'3C_dosing'& 
                                        Scheme %in% input$'3C_scheme'))
    Ctrough_3C_selected<-reactive(filter(Ctrough_3C,
                                         Dosing %in% input$'3C_dosing'& 
                                           Scheme %in% input$'3C_scheme'))
    DCmin_3C_selected<-reactive(filter(DCmin_3C,
                                       Dosing %in% input$'3C_dosing'& 
                                         Scheme %in% input$'3C_scheme'))
    TumorR_3C_selected<-reactive(filter(TumorR_3C,
                                        Dosing %in% input$'3C_dosing'& 
                                          Scheme %in% input$'3C_scheme'))
    #================= 3C Pop plots =========================
    ...
    output$'3C_Ctrough'<-renderPlotly({
      plot_Ctrough_3C<-ggplot(Ctrough_3C_selected(),aes(x = Scheme,y = Ctrough))+
        geom_violin(aes(fill = Scheme),scale = "width",alpha = 0.4)+
        facet_grid(.~Dosing)+
        geom_boxplot(aes(fill = Scheme),width = 0.25,alpha = 0.4)+
    ...
    output$'3C_DCmin'<-renderPlotly({
      plot_DCmin_3C<-ggplot(DCmin_3C_selected(),aes(x = Scheme,y = DrugComplex_min))+
        geom_violin(aes(fill = Scheme),scale = "width",alpha = 0.4)+
        facet_grid(.~Dosing)+
        geom_boxplot(aes(fill = Scheme),width = 0.25,alpha = 0.4)+
      ...
      plot_DCmin_3C<-plot_DCmin_3C%>%plotly_build()%>%
        layout(yaxis = list(title = list(text = 'Min Drug Complex Ratio (%)',
                                         standoff = 2,
                                         font = list(size = 25))),
               xaxis = list(title = list(text = '',standoff = 10,
                                         font = list(size = 10)))
    ...
    output$'3C_TumorR'<-renderPlotly({
      plot_TumorR_3C<-ggplot(TumorR_3C_selected(),aes(x = Scheme,y = Tumor_Ratio))+
        geom_violin(aes(fill = Scheme),scale = "width",alpha = 0.4)+
        facet_grid(.~Dosing)+
        geom_boxplot(aes(fill = Scheme),width = 0.25,alpha = 0.4)+
      ...
      plot_TumorR_3C<-plot_TumorR_3C%>%plotly_build()%>%
        layout(yaxis = list(title = list(text = 'Tumor Change Ratio',
                                         standoff = 2,
                                         font = list(size = 25))),
               xaxis = list(title = list(text = '',standoff = 10,
                                         font = list(size = 10)))
