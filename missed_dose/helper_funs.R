...
process_data_2C <- function(data_list,
  ...
  for (item in data_list)
  {
    ...
    Cmax_tab <-
      cbind(Cmax_tab, as.vector(get(names(item)[2], item)))
    Ctrough_tab <-
      cbind(Ctrough_tab, as.vector(get(names(item)[3], item)))
  }
...
process_data_retake_2C <- function(data_list,
  ...
  for (item in data_list)
  {
    ...
    AUC_tab <-
      rbind(AUC_tab, retake_table(item, 1, retake_tag, dosing_method))
    Cmax_tab <-
...
process_data_3C <- function(data_list,
  ...
  for (item in data_list)
  {
    ...
    Ctrough_tab <-
      cbind(Ctrough_tab, as.vector(get(names(item)[3], item)))
    DCmin_tab <-
      cbind(DCmin_tab, as.vector(get(names(item)[4], item)))
    TumorR_tab <-
      cbind(TumorR_tab, as.vector(get(names(item)[5], item)))
  }
  AUC_tab <- as.data.frame(AUC_tab)
  Cmax_tab <- as.data.frame(Cmax_tab)
  Ctrough_tab <- as.data.frame(Ctrough_tab)
...
process_data_retake_3C <- function(data_list,
                                   dosing_tag,
                                   retake_tag) {
  AUC_tab <- c()
  Cmax_tab <- c()
